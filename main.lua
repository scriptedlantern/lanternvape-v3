...
local C={Name="LanternVape",Version="2.01",LoadingTime=2.2,Orange=Color3.fromRGB(220,115,35),OrangeDark=Color3.fromRGB(145,68,20),Black=Color3.fromRGB(8,8,8),Dark=Color3.fromRGB(14,14,14),Darker=Color3.fromRGB(21,21,21),White=Color3.fromRGB(235,235,235),Gray=Color3.fromRGB(145,145,145)}
...
-- Loading is created first and fills the entire display.
...
-- Safe area for the actual interface.
...
for i,d in ipairs(items) do
 ...
 b.MouseEnter:Connect(function()tw(b,.12,{BackgroundColor3=C.OrangeDark})end);b.MouseLeave:Connect(function()tw(b,.12,{BackgroundColor3=C.Darker})end)
 menuButtons[d[1]]=b
end
...
-- Settings: category visibility + mobile button visibility.
...
local function setAccent(color)
 ...
 title.TextColor3=color;bar.BackgroundColor3=color
end
...
for _,name in ipairs({"Profiles","Targets","Keybinds","About"}) do
 menuButtons[name].MouseButton1Click:Connect(function()side.Visible=false end)
end
...
-- Loading can never block the construction above.
