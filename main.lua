...
local C={Name="LanternVape",Version="2.00",LoadingTime=2.2,Orange=Color3.fromRGB(220,115,35),OrangeDark=Color3.fromRGB(145,68,20),Black=Color3.fromRGB(8,8,8),Dark=Color3.fromRGB(14,14,14),Darker=Color3.fromRGB(21,21,21),White=Color3.fromRGB(235,235,235),Gray=Color3.fromRGB(145,145,145)}
...
L.Size=UDim2.fromScale(1,1);L.BackgroundColor3=C.Black;L.BackgroundTransparency=.08;L.BorderSizePixel=0;L.ZIndex=1000;L.Parent=G
local LT=Instance.new("Frame")
LT.Size=UDim2.fromScale(1,1);LT.BackgroundColor3=C.Orange;LT.BackgroundTransparency=.96;LT.BorderSizePixel=0;LT.ZIndex=1001;LT.Parent=L
local title=Instance.new("TextLabel")
title.Size=UDim2.new(1,0,0,50);title.Position=UDim2.new(0,0,.5,-35);title.BackgroundTransparency=1;title.Text="LOADING LANTERNVAPE..";title.TextColor3=C.Orange;title.TextSize=25;title.Font=Enum.Font.GothamBold;title.ZIndex=1002;title.Parent=L
local lb=Instance.new("Frame")
lb.Size=UDim2.fromOffset(240,4);lb.AnchorPoint=Vector2.new(.5,0);lb.Position=UDim2.new(.5,0,.5,25);lb.BackgroundColor3=C.Darker;lb.BorderSizePixel=0;lb.ZIndex=1002;lb.Parent=L;cr(lb,10)
local bar=Instance.new("Frame")
bar.Size=UDim2.new(0,0,1,0);bar.BackgroundColor3=C.Orange;bar.BorderSizePixel=0;bar.ZIndex=1003;bar.Parent=lb;cr(bar,10)

-- Safe area below Roblox top bar
local Safe=Instance.new("Frame")
Safe.Size=UDim2.new(1,0,1,-24);Safe.Position=UDim2.fromOffset(0,24);Safe.BackgroundTransparency=1;Safe.Parent=G
local Main=Instance.new("Frame")
Main.Size=UDim2.new(1,-30,1,-30);Main.Position=UDim2.fromOffset(15,15);Main.BackgroundTransparency=1;Main.Visible=false;Main.Parent=Safe
local scale=Instance.new("UIScale");scale.Parent=Main

-- Header
local brand=Instance.new("TextLabel")
brand.Size=UDim2.fromOffset(190,42);brand.Position=UDim2.fromOffset(4,4);brand.BackgroundColor3=C.Black;brand.BackgroundTransparency=.05;brand.BorderSizePixel=0;brand.Text=C.Name;brand.TextColor3=C.White;brand.TextSize=18;brand.Font=Enum.Font.GothamBold;brand.TextXAlignment=Enum.TextXAlignment.Left;brand.Parent=Main;cr(brand,5)
...
-- Version gets its own non-overlapping area
local ver=Instance.new("TextLabel")
ver.Size=UDim2.fromOffset(80,24);ver.AnchorPoint=Vector2.new(1,0);ver.Position=UDim2.new(1,-55,0,13);ver.BackgroundTransparency=1;ver.Text="v"..C.Version;ver.TextColor3=C.Gray;ver.TextSize=11;ver.Font=Enum.Font.Gotham;ver.TextXAlignment=Enum.TextXAlignment.Right;ver.Parent=Main

-- Search bar with icon
local searchFrame=Instance.new("Frame")
searchFrame.Size=UDim2.fromOffset(230,34);searchFrame.AnchorPoint=Vector2.new(.5,0);searchFrame.Position=UDim2.new(.5,0,0,8);searchFrame.BackgroundColor3=C.Black;searchFrame.BackgroundTransparency=.05;searchFrame.BorderSizePixel=0;searchFrame.Parent=Main;cr(searchFrame,6);st(searchFrame,C.Orange,.8,1)
local searchIcon=iconButton(searchFrame,"SearchIcon",UDim2.fromOffset(28,28),UDim2.fromOffset(3,3),Icons.Search,"⌕",5)
searchIcon.BackgroundTransparency=1
local search=Instance.new("TextBox")
search.Size=UDim2.new(1,-36,1,0);search.Position=UDim2.fromOffset(34,0);search.BackgroundTransparency=1;search.BorderSizePixel=0;search.PlaceholderText="Search..";search.PlaceholderColor3=Color3.fromRGB(130,130,130);search.Text="";search.TextColor3=C.White;search.TextSize=13;search.Font=Enum.Font.Gotham;search.ClearTextOnFocus=false;search.TextXAlignment=Enum.TextXAlignment.Left;search.Parent=searchFrame

-- Category grid
local content=Instance.new("ScrollingFrame")
content.Size=UDim2.new(1,-8,1,-62);content.Position=UDim2.fromOffset(4,58);content.BackgroundTransparency=1;content.BorderSizePixel=0;content.ScrollBarThickness=3;content.ScrollBarImageColor3=C.Orange;content.AutomaticCanvasSize=Enum.AutomaticSize.Y;content.ScrollingDirection=Enum.ScrollingDirection.Y;content.Parent=Main
...
local cats={"Combat","Blatant","External","Rendering","Extra"}
local panels={}
...
side.Size=UDim2.fromOffset(235,385);side.Position=UDim2.fromOffset(8,58);side.BackgroundColor3=C.Black;side.BackgroundTransparency=.01;side.BorderSizePixel=0;side.Visible=false;side.ZIndex=50;side.Parent=Main;cr(side,10);st(side,C.Orange,.55,1)
local sh=Instance.new("TextLabel")
sh.Size=UDim2.new(1,0,0,54);sh.BackgroundColor3=C.Darker;sh.BorderSizePixel=0;sh.Text="  LANTERNVAPE  •  MENU";sh.TextColor3=C.White;sh.TextSize=14;sh.Font=Enum.Font.GothamBold;sh.TextXAlignment=Enum.TextXAlignment.Left;sh.ZIndex=51;sh.Parent=side;cr(sh,10)
...
local pad=Instance.new("UIPadding");pad.PaddingTop=UDim.new(0,88);pad.PaddingLeft=UDim.new(0,10);pad.PaddingRight=UDim.new(0,10);pad.Parent=side
local items={{"Settings","Settings","⚙"},{"Profiles","Profiles","♙"},{"Targets","Targets","◎"},{"Themes","Themes","◆"},{"Keybinds","Keybinds","⌨"},{"About","About","?"}}
local menuButtons={}
for i,d in ipairs(items) do
 ...
 b.MouseEnter:Connect(function()tw(b,.12,{BackgroundColor3=C.OrangeDark})end)
 b.MouseLeave:Connect(function()tw(b,.12,{BackgroundColor3=C.Darker})end)
 menuButtons[d[1]]=b
end

-- Floating mobile button, top-right
...
search:GetPropertyChangedSignal("Text"):Connect(function()
 ...
 for n,p in pairs(panels)do p.Visible=q=="" or n:lower():find(q,1,true)~=nil end
end)
...
local function makeToggle(parent,text,y,initial,callback)
 ...
 local state=initial
 b.MouseButton1Click:Connect(function()state=not state;dot.BackgroundColor3=state and C.Orange or C.Gray;callback(state)end)
 return b
end
local y=70
for _,n in ipairs(cats) do
 makeToggle(settings,"Show "..n,y,true,function(v)panels[n].Visible=v end)
 y=y+46
end
makeToggle(settings,"Show LanternVape button",y,true,function(v)toggle.Visible=v and UIS.TouchEnabled end)
...
local tt=Instance.new("TextLabel");tt.Size=UDim2.new(1,-24,0,50);tt.Position=UDim2.fromOffset(12,8);tt.BackgroundTransparency=1;tt.Text="THEMES";tt.TextColor3=C.White;tt.TextSize=18;tt.Font=Enum.Font.GothamBold;tt.TextXAlignment=Enum.TextXAlignment.Left;tt.ZIndex=101;tt.Parent=themes
local tc=iconButton(themes,"CloseThemes",UDim2.fromOffset(36,36),UDim2.new(1,-48,0,12),Icons.Close,"×",102)
local colors={{"Orange",Color3.fromRGB(220,115,35)},{"Red",Color3.fromRGB(220,55,55)},{"Purple",Color3.fromRGB(155,90,220)},{"Blue",Color3.fromRGB(70,135,235)},{"Green",Color3.fromRGB(70,190,105)},{"Pink",Color3.fromRGB(230,90,160)},{"White",Color3.fromRGB(220,220,220)}}
for i,d in ipairs(colors) do
 local b=Instance.new("TextButton");b.Size=UDim2.fromOffset(112,42);b.Position=UDim2.fromOffset(16+((i-1)%3)*124,70+math.floor((i-1)/3)*52);b.BackgroundColor3=C.Darker;b.BorderSizePixel=0;b.Text=d[1];b.TextColor3=C.White;b.TextSize=12;b.Font=Enum.Font.GothamSemibold;b.AutoButtonColor=false;b.ZIndex=101;b.Parent=themes;cr(b,7)
 local sw=Instance.new("Frame");sw.Size=UDim2.fromOffset(12,12);sw.Position=UDim2.fromOffset(8,15);sw.BackgroundColor3=d[2];sw.BorderSizePixel=0;sw.ZIndex=102;sw.Parent=b;cr(sw,6)
 b.TextXAlignment=Enum.TextXAlignment.Right
 local pad2=Instance.new("UIPadding");pad2.PaddingRight=UDim.new(0,10);pad2.Parent=b
 b.MouseButton1Click:Connect(function()
  C.Orange=d[2]
  C.OrangeDark=d[2]:Lerp(Color3.new(0,0,0),.35)
  for _,o in ipairs(G:GetDescendants()) do
   if o:IsA("UIStroke") and o.Color~=C.White then o.Color=C.Orange end
   if o:IsA("Frame") and (o==ba or o==a or o==bar or o==LT) then o.BackgroundColor3=C.Orange end
  end
  if toggle then local s=toggle:FindFirstChildOfClass("UIStroke");if s then s.Color=C.Orange end end
 end)
end
menuButtons.Themes.MouseButton1Click:Connect(function()side.Visible=false;themes.Visible=true end)
tc.MouseButton1Click:Connect(function()themes.Visible=false end)

-- Close overlays when main closes
close.MouseButton1Click:Connect(function()settings.Visible=false;themes.Visible=false end)
...
-- Responsive scaling. Five columns remain fixed.
...
-- Loading
TweenService:Create(bar,TweenInfo.new(C.LoadingTime,Enum.EasingStyle.Quad,Enum.EasingDirection.Out),{Size=UDim2.new(1,0,1,0)}):Play()
task.wait(C.LoadingTime)
Main.Visible=true
if UIS.TouchEnabled then toggle.Visible=true end
tw(L,.35,{BackgroundTransparency=1});tw(LT,.35,{BackgroundTransparency=1});tw(title,.35,{TextTransparency=1});tw(lb,.35,{BackgroundTransparency=1});tw(bar,.35,{BackgroundTransparency=1})
task.wait(.4)
if L.Parent then L:Destroy()end
print("["..C.Name.."] Loaded v"..C.Version)